...
from doc_shelf.exceptions import PDFExtractionError
...
def extract(pdf_path: str) -> ExtractedDocument:
    ...
    if not text.strip():
        raise PDFExtractionError(
            "No text could be extracted. The PDF may be image-only and require OCR."
        )
...
def _extract_metadata(doc: pymupdf.Document) -> dict:
    """Extract useful PDF metadata fields."""
