...
class DocumentShelvesUpdate(BaseModel):
    shelf_ids: list[str]
...
def update_shelf(shelf_id: str, body: ShelfUpdate, request: Request) -> dict:
    output_dir = request.app.state.output_dir
    try:
        return library.rename_shelf(shelf_id, body.name, output_dir, name_ja=body.name_ja)
    except StorageError as e:
...
@router.put("/documents/{document_id}/shelves")
def set_document_shelves(
    document_id: str,
    body: DocumentShelvesUpdate,
    request: Request,
) -> dict:
    output_dir = request.app.state.output_dir
    try:
        library.assign_document_to_shelves(document_id, body.shelf_ids, output_dir)
        return {"ok": True}
    except StorageError as e:
        raise HTTPException(status_code=404, detail=str(e))


@router.post("/documents/{document_id}/shelves/{shelf_id}")
def add_document_to_shelf(document_id: str, shelf_id: str, request: Request) -> dict:
    output_dir = request.app.state.output_dir
    try:
        library.add_document_to_shelf(document_id, shelf_id, output_dir)
        return {"ok": True}
    except StorageError as e:
        raise HTTPException(status_code=404, detail=str(e))


@router.delete("/documents/{document_id}/shelves/{shelf_id}")
def remove_document_from_shelf(document_id: str, shelf_id: str, request: Request) -> dict:
    output_dir = request.app.state.output_dir
    try:
        library.remove_document_from_shelf(document_id, shelf_id, output_dir)
        return {"ok": True}
    except StorageError as e:
        raise HTTPException(status_code=404, detail=str(e))
